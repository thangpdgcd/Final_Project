...
const { PromoVouchers, Vouchers, VoucherAuditLogs, Orders, sequelize } = models
...
export const voucherApplyService = {
  apply: async ({ code, orderValue, userId }) => {
    ...
    // 1) Promo vouchers (promo_vouchers) — global marketing vouchers
    const promoRow = await PromoVouchers.findOne({ where: { code: voucherCode } })
    if (promoRow) {
      const v = promoRow.toJSON ? promoRow.toJSON() : promoRow
    ...
    // 2) Manual vouchers (vouchers) — created by staff/admin, usually assigned to a user
    const manualRow = await Vouchers.findOne({ where: { code: voucherCode } })
    if (!manualRow) throw new AppError("Voucher not found", 404)

    const mv = manualRow.toJSON ? manualRow.toJSON() : manualRow

    if (mv.deletedAt) throw new AppError("Voucher is inactive", 400)
    if (String(mv.status ?? "").toLowerCase() !== "active") throw new AppError("Voucher is inactive", 400)

    if (mv.expiresAt && new Date(mv.expiresAt).getTime() <= now.getTime()) {
      throw new AppError("Voucher is expired", 400)
    }

    const maxUsage = Math.max(1, Math.trunc(Number(mv.maxUsage ?? 1) || 1))
    const usedCount = Math.max(0, Math.trunc(Number(mv.usedCount ?? 0) || 0))
    if (usedCount >= maxUsage) throw new AppError("Voucher usage limit reached", 400)

    // Manual vouchers should not leak across accounts: if assigned, require same userId
    if (mv.userId != null) {
      if (uid == null) throw new AppError("userId is required for this voucher", 400)
      if (Number(mv.userId) !== Number(uid)) throw new AppError("Voucher is not applicable for this user", 403)
    }

    const { discountAmount, finalPrice } = computeDiscount({
      discountType: mv.type === "percent" ? "percentage" : "fixed",
      discountValue: mv.value,
      maxDiscountValue: mv.maxDiscountValue,
      orderValue: ov,
    })

    await sequelize.transaction(async (tx) => {
      const [affected] = await Vouchers.update(
        {
          usedCount: sequelize.literal("used_count + 1"),
          usedAt: now,
        },
      ...
      await VoucherAuditLogs.create(
        {
          ...
          action: "redeem",
          userId: uid ?? null,
          meta: { code: voucherCode, orderValue: ov, discountAmount },
        },
        { transaction: tx },
      )
    })
    ...
    return {
      ...
      voucher: {
        code: voucherCode,
        discountType: mv.type === "percent" ? "percentage" : "fixed",
        discountValue: Number(mv.value),
        maxDiscountValue: mv.maxDiscountValue != null ? Number(mv.maxDiscountValue) : null,
        source: "manual",
      },
    }
  },
