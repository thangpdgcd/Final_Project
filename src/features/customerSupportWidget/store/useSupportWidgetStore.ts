...
type SupportWidgetState = {
  ...
  appendOptimisticText: (conversationId: number, myUserId: number, content: string) => { clientId: string; tempId: string };
...
export const useSupportWidgetStore = create<SupportWidgetState>()(
  ...
  setMessages: (messages) => set({ messages: [...messages].sort((a, b) => a.createdAt - b.createdAt) }),
  ...
  appendIncoming: (payload) => {
    ...
    set((state) => {
      ...
      const nextMessages = alreadyHas ? state.messages : [...state.messages, message].sort((a, b) => a.createdAt - b.createdAt);
  ...
  markOptimisticFailed: (clientId) => {
    set((state) => ({
      messages: state.messages.map((m) =>
        m.optimistic?.clientId === clientId ? { ...m, optimistic: { ...m.optimistic, status: 'failed' as const } } : m,
      ),
    }));
  },
  ...
  reconcileOptimisticIfMatch: (incoming) => {
    ...
    set((state) => {
      ...
      const next = updated.map((m, i) =>
        i === realIdx
          ? { ...incoming, optimistic: { clientId: target.optimistic!.clientId, status: 'sent' as const } }
          : m,
      );
