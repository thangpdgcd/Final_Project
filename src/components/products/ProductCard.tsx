  ...
  ({ product, title, description, price, imageSrc, onOpen, onAddToCart }) => {
    ...
    const badges = useMemo(() => {
      ...
      if (Number(product.price) > 35000) {
        list.push({ label: 'Best Seller', variant: 'bestSeller' });
      }
      if ((product.name || '').trim().toLowerCase() === 'butter croissant') {
        list.push({ label: 'New', variant: 'new' });
      }
      return list;
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-1 flex-col mt-5">
          <h3 className="line-clamp-1 text-lg font-bold text-stone-900 dark:text-stone-100" style={{ fontFamily: 'var(--font-display), serif' }}>
            {title}
          </h3>
          <p className="mt-1.5 flex-1 line-clamp-2 text-sm leading-relaxed text-stone-500 dark:text-stone-400">
            {description}
          </p>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-stone-100 dark:bg-stone-800 px-4 py-3 text-sm font-bold text-stone-900 dark:text-stone-100 transition-all hover:bg-stone-800 hover:text-white dark:hover:bg-stone-200 dark:hover:text-stone-900"
