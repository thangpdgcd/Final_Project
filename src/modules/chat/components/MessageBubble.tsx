...
export const MessageBubble: React.FC<Props> = ({ message, isMine }) => {
  const base = 'max-w-[82%] rounded-2xl px-3 py-2 border shadow-[0_12px_30px_rgba(0,0,0,0.22)]';
  ...
  return (
    <div className={['flex', isMine ? 'justify-end' : 'justify-start'].join(' ')}>
      <div className={[base, isMine ? mine : theirs].join(' ')}>
        <div className="text-[11px] opacity-70">{isMine ? 'You' : message.sender.name}</div>
        {message.type === 'text' ? (
