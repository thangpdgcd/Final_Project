...
const ensureOrderTables = async () => {
  if (!OrderMeta || !OrderMessages) return;
  if (!ensureTablesPromise) {
    ensureTablesPromise = Promise.all([OrderMeta.sync(), OrderMessages.sync()]);
  }
...
const listOrderMessages = async ({ orderId, actor }) => {
  ...
  const rows = await OrderMessages.findAll({
...
const createOrderMessage = async ({ orderId, actor, message }) => {
  ...
  const row = await OrderMessages.create({
    orderId: order.orderId,
    senderId: actor.id,
