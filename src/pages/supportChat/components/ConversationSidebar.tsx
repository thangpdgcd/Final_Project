...
import type { SupportChatConversation } from '@/types/supportChat/types';
import { ConversationRow } from './ConversationRow';
import { SidebarSkeleton } from './Skeletons';
import { useOnlineStaff } from '@/hooks/supportChat/useOnlineStaff';
  ...
  return (
    <div className="h-full flex flex-col border-r border-white/10 bg-black/40">
      <div className="p-3 space-y-2">
        ...
        {staffJoinSlot ? <div>{staffJoinSlot}</div> : null}

        {onContactSupport ? (
          <button
            type="button"
            onClick={onContactSupport}
            className="w-full rounded-2xl bg-[#0866ff] text-white text-sm font-semibold py-2 hover:brightness-110 transition"
          >
            Contact support
          </button>
        ) : null}
