...
const Login = () => {
  ...
  const handleGoogleToken = async (token) => {
    ...
    try {
      ...
      const { accessToken, user } = await authService.googleLogin(token)
          ...
          <div className="p-6 sm:p-8">
            ...
            {!canUseGoogle ? (
              <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-100">
                Missing <span className="font-semibold">VITE_GOOGLE_CLIENT_ID</span>. Please set it in your{' '}
                <span className="font-semibold">.env</span> and restart the dev server.
              </div>
            ) : (
              <>
                <div className="flex flex-col items-center justify-center gap-3">
                  <GoogleButton onToken={handleGoogleToken} loading={loading} disabled={!canUseGoogle || loading} />
